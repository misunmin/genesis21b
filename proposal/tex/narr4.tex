...
As illustrated in \label{fig:fm1}, the {\em Phase I} goal is to develop a training
workflow to develop AI surrogates for thermal-fluid solutions.   Initially, we
consider training sets based on time-independent mean flow fields that are already
available or a readily generated from Nek5000/RS and MFEM using direct numerical
...
angles, etc.)   The inference and encoding process uses TensorFlow on GPUs.  
As products of DOE's Exascale Computing Project, both Nek5000/RS and MFEM run
on NVIDIA, AMD, and Intel accellerators, so there are no GPU portability issues.
Because time-averaging smooths the solution, it is possible to down-sample the mesh
and solution data before encoding.  Moreover, we envision load solution {\em 
spatial derivatives} during the training in order to avoid differentating
the surrogate solution.  Derivatives are critical to pressure-drop and
thermal flux QOIs.  Having them generated by the surrogate takes significant
pressure off of having a high-quality mesh for the cases of interest during
the design cycle.
Below, we identify the effort breakdown for the project.
...
%\end{wrapfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
The Argonne team has extensive experience in large eddy simulation (LES) of turbulent flow in liquid metal fast reactors, including everything from isolated subassemblies \cite{fischer08a,Dai2025WireWrapped61Pin,shaver37pin,shavernacie} to full cores~\cite{sc22,gb23}.
The majority of these simulation datasets are still available and summarized in the nuclear engineering literature. 
In particular we have ready access to full-3D high-fidelity datasets for adiabatic flow in a 61-pin assembly covering a range of Reynolds numbers \cite{Dai2025WireWrapped61Pin} and heated flow in a 19-pin assembly under multiple flow and heating conditions \cite{shavernacie}. 
All of these simulation datasets have corresponding experimental datasets that demonstrate their validity.
Additional data is currently being generated using NekRS on existing compute resources on OLCF Frontier, ALCF Aurora, and the NE Divison cluster, {\em Nek5k}, as part of ongoing studies.  
This includes a series of 7-pin reflector assemblies and additional 37-pin simulations similar to those performed by Shaver \cite{shaver37pin}.
The latest (and most detailed) experimental data for wire-wrapped bundles is available in \cite{kaeri37pin}, which will also be used for model training.
The generation of wire-wrapped fuel bundle geometries has been fully automated so that model training at new points in the wire-wrap diameter / pitch space is straightforward.  
We are able to generate data for single-pin, fully periodic domains to cover a broad range of conditions quickly which can be integrated in to the training data to control uncertainty.
