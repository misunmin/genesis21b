...
as well as neural-operator architectures for physics-consistent surrogate learning
which we will investigate in Phase II. 
...
led by Argonne with contributions from all partners. In addition to wire-wrapped
reactor bundles, two complementary applications will be considered to expand the scope
and generality of the approach. 
%
The first application involves {\bf inertial confinement fusion}, where strongly transient,
...
%
The second application targets a {\bf well-characterized heat-transfer enhancement problem}
involving wire-coil inserts in cooling channels, where experimental data~\cite{collins22}
demonstrate up to fourfold increases in heat transfer accompanied by well-quantified pressure penalties.
This problem provides an ideal benchmark for model development and validation
due to the sensitivity of the QOI (typically the Nusselt number), its rich 
parameter space--including wire diameter, pitch, and Reynolds number--and the 
strong agreement between simulations and experiments, and relatively low cost 
for simulation data.
}
...
design with $10$--$20\times$ acceleration and improved performance
(e.g., heat-transfer versus pressure-drop trade-off) on a
representative heat-exchanger system.
