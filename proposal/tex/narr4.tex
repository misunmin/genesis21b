...
\begin{figure}[b]
    \centering
    \includegraphics[width=0.50\linewidth]{../figs/method.png}
%
...
% The central objective of this {\bf Phase I effort} is to unify these heterogeneous datasets 
% into a common, geometry-aware representation and demonstrate a {\bf closed-loop workflow} 
% that enables rapid prediction, design exploration, and optimization of 
% reactor-relevant thermal–fluid systems.
% }
 
This project will develop a {\bf foundation-model-driven, agentic-AI framework}
\cite{Vinuesa2026XAI} for the prediction, optimization, and control of
thermal-fluid transport in energy systems, with primary emphasis on 
fission and fusion applications.  
  These fluid systems are central to DOE’s advanced energy programs and 
are characterized by complex turbulent flow, strong geometric sensitivity, 
and stringent performance and safety constraints.
...
licensing workflows) and MFEM (the 2025 Gordon Bell Winner) with extensive
experimental datasets from both open literature and proprietary industry sources. 
   The central objective of this {\bf Phase I effort} is to unify these heterogeneous 
datasets into a common (mesh-based) geometry-aware representation and to demonstrate rapid,
accurate, surrogate solution generation that will be applied to design and
optimization of thermal-fluid systems.
%%    The central objective of this {\bf Phase I effort} is to unify these heterogeneous 
%% datasets into a common geometry-aware representation and to demonstrate a
%% {\bf closed-loop workflow} for rapid design and optimization of thermal-fluid systems.

Thermal–fluid transport governs the performance of a wide range of energy systems. 
In nuclear fission and fusion reactors, fluid motion transports thermal energy 
from core processes (fission/fusion reactions) to heat exchangers, where working 
fluids drive turbines or other power-generation devices. Combustion systems and 
inertially confined fusion (ICF) platforms similarly depend on controlled fluid 
motion to initiate and sustain energy-producing reactions. 
However, {\bf system- and device-level optimization—particularly when involving 
geometric or topology modifications—typically requires tens to hundreds of high-fidelity 
simulations}, each taking days to months on high-performance computing (HPC) platforms. 
The computational burden arises from complex geometries, broad ranges of timescales 
(e.g., convective flow versus thermal diffusion in solids), and stringent 
accuracy requirements.

The overall goal of this project, to be realized in Phase II, is to radically 
transform the design cycle for energy-system components by providing reliable, 
high-accuracy AI-driven surrogate models tightly coupled with state-of-the-art 
design optimization methods. The Phase I test problems are selected to establish 
feasibility across representative DOE-relevant applications, including reactor-core 
flows and inertially confined fusion (ICF) miniapps, while liquid-metal 
...
As illustrated in Fig. 1, Phase I test cases include:
   ...
   (ii) a \textbf{miniapp for inertially confined fusion}; and
...
\tiny \begin{verbatim}
Misun/Elia/Dillon: Provide a clear and concise statement of the specific objectives of the proposed project.  
Address how the objectives align with the chosen focus topic and lead to an AI advantage.
\end{verbatim} 
\normalsize
\input objective
...
As illustrated in Fig. \ref{fig:fm1}, the {\bf Phase I} goal is to develop a training
workflow to build AI surrogates for thermal-fluid solutions.   Initial
training sets will be based on time-independent {mean-flow} fields that are already
...
solution variables velocity, pressusure, and temperature.   Cases with 
varying geometry or parameters (e.g., Reynolds number) are fed into an
encoder, which builds the latent space.  Once trained, a new geometry (mesh)
can be fed into the encoder to generate a surrogate solution within a few
seconds.  This process has been tested with success using LES-based flow data
for the NASA full-aircraft test model at different angles of attack, slat
angles, etc., using datasets of size $n \approx 50$M.
The encoding process uses TensorFlow on GPUs (several days on four to eight H200s)
and inference takes about 30 seconds.  As products of DOE's Exascale Computing
Project, both Nek5000/RS and MFEM run on NVIDIA, AMD, and Intel accelerators,
so there are no GPU portability issues.  
...
Training will leverage DOE exascale-ready workflows and GPU-enabled datasets
generated using {\rm Nek5000/RS} and {\rm MFEM}. The resulting surrogate model
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Original taxonomy figure data is in latex
%\input taxonomy.tex 
%\input taxonomy_new.tex 
%\input taxonomy_new2.tex 
%\input taxonomy_new3.tex 
%\input taxonomy_new4.tex 
\input taxonomy_new5.tex 
 \bigskip
\input taxonomy_table.tex 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

 \begin{wrapfigure}{r}{0.45\textwidth} 
 \centering
 \includegraphics[width=0.44\textwidth]{../figs/taxonomy2.png}
 \vspace{-2pt}
...
%\begin{wrapfigure}{r}{0.45\textwidth} 
%\centering
%\includegraphics[width=0.44\textwidth]{../figs/taxonomy.png}
%vspace{-2pt}
%\caption{\small Taxonomy of architecture components used in physics-informed
...
% geometry-aware foundation model for thermal-fluid prediction, led by ANL, UIUC,
% and PSU.  Building on prior work on foundation
% models~\cite{Vishwasrao2026AgenticPDE}, as well as simulation and experimental
% data from wire-wrapped bundles \cite{Dai2025WireWrapped61Pin,kaeri37pin}, and
% related configurations such as spacer grids, pebble beds, and twisted tubes,
% results will be mapped to a common latent space independent of mesh topology.
% %
...
% heat transfer and pressure losses.  The model will combine
% graph-basedrepresentations of geometry, variational autoencoders for
% compressing high-dimensional flow fields
% \cite{Vinuesa2022MLCFD,SoleraRico2024BetaVAE}, and conditional latent diffusion
% \cite{Vishwasrao2025DiffSPORT} to learn the latent distribution across
% operating regimes. 
% %
...
The second component, led by the University of Michigan, develops a 
{\bf latent-space optimization and control framework}, targeting both steady and
transient operating regimes.
This effort will employ a hybrid strategy combining {\rm Bayesian optimization}~\cite{Morita2022BOCFD},
{\rm derivative-free optimization methods}, and {\rm reinforcement learning}~\cite{Font2025DRLFlowControl}.
...
and operating conditions. The {rm agentic AI system} will orchestrate this process by dynamically
...
%
The framework will operate in a closed-loop workflow, in which candidate designs generated
in latent space are selectively validated using high-fidelity simulations.
...
The third component focuses on {\bf cross-domain validation and integration into the
Genesis Mission ecosystem}, with contributions from all partners. 
In addition to wire-wrapped reactor bundles, two complementary applications
will be considered to expand the scope and generality of the approach. 
%
The first application involves {\rm inertial confinement fusion}, where strongly transient,
multiphysics dynamics arise from laser-driven compression of fuel pellets.
This provides a stringent test of the model’s ability to capture rapidly evolving physical regimes.
%
The second application targets a {\rm well-characterized heat-transfer enhancement problem}
involving wire-coil inserts in cooling channels, where experimental data~\cite{collins22}
demonstrate up to fourfold increases in heat transfer for fixed flow rates.
   This problem is particularly valuable for model development and validation
because of the sensitivity of the QOI (typically the Nusselt number), the rich
...
{\bf [Tzanio/Vlad - possible to integrate into above? Happy to discuss.]}
...
workflows.
  ...
  Together, these Phase I applications provide wide coverage of 
...
(e.g., a 32X gain compared to 2nd-order finite volume methods 
\cite{Min2024Wind,Tomboulides2024ABL}).  Nek5000/RS has
been extensively used for reactor thermal-hydraulics and is part of
NRC-accepted workflows \cite{URL?}. 
...
{\bf Shorten, but is following Instructions.}
...
%      \put(0.0,0.0){\includegraphics[height=1.5in]{../figs/collins_02_nek_nu3.png}}
%   \end{picture}}
...
% \centering {\setlength{\unitlength}{1.00in}
% \begin{picture}(2.0,1.5)(0,0)
%     \put(0.0,0.0)\includegraphics[width=0.4\textwidth]{../figs/collins_02_nek_nu3.png}
%    \end{picture}
%  }
%    \caption{\label{fig:collins}
%    Experimental-Nek5000/RS data comparison of Nusselt number
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{wrapfigure}{rb}{0.35\textwidth}

\vspace{-2pt}
\centering
\includegraphics[width=0.34\textwidth]{../figs/dai_data.png}

%vspace{-2pt}
\caption{\label{fig:blocked}
Experimental-NekRS data comparison of friction factor
vs. Reynolds number for a wire-wrapped bundle with blockage \cite{Dai2025WireWrapped61Pin}.
}
%\vspace{-7pt}
...
%\vspace{-2pt}
%\centering
%\includegraphics[width=0.34\textwidth]{../figs/nacie_temp.png}

%vspace{-2pt}
%\caption{\label{fig:nacie} Experimental-NekRS data comparison of pin wall temperatures for a 19-pin wire-wrapped bundle \cite{shavernacie}.
%}
%\vspace{-7pt}
...
turbulent flow in liquid metal fast reactors, including everything from
isolated subassemblies
\cite{fischer08a,Dai2025WireWrapped61Pin,shaver37pin,shavernacie} to full
cores~\cite{sc22,gb23}.  The majority of these simulation datasets are still
available and summarized in the nuclear engineering literature.  In particular
we have ready access to full-3D high-fidelity datasets for adiabatic flow in a
61-pin assembly covering a range of Reynolds numbers
\cite{Dai2025WireWrapped61Pin} and heated flow in a 19-pin assembly under
multiple flow and heating conditions \cite{shavernacie}.  All of these
simulation datasets have corresponding experimental datasets that demonstrate
their validity.  Additional data is currently being generated using NekRS on
existing compute resources on OLCF Frontier, ALCF Aurora, and the NE Divison
cluster, {\em Nek5k}, as part of ongoing studies.  This includes a series of
7-pin reflector assemblies and additional 37-pin simulations similar to those
performed by Shaver \cite{shaver37pin}.  The latest (and most detailed)
experimental data for wire-wrapped bundles is available in \cite{kaeri37pin},
which will also be used for model training.  The generation of wire-wrapped
fuel bundle geometries has been fully automated so that model training at new
points in the wire-wrap diameter / pitch space is straightforward.  We are able
to generate data for single-pin, fully periodic domains to cover a broad range
of conditions quickly which can be integrated in to the training data to
control uncertainty.
%This validity of this simulation data is backed up through extensive comparisons with experiments.  

Because time-averaging smooths the solution, it is possible to down-sample the
mesh and solution data before encoding.  Because derivatives are critical to
pressure-drop and thermal flux QOIs, {\em spatial derivatives} will also be
encoded during training in order to bypass the need for a fine mesh when
differentiating the surrogate solution.  
...
\vspace{-2pt}
\centering
\includegraphics[width=0.34\textwidth]{../figs/collins_02_nek_nu3_new.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent
%{\bf Dillon, Elia, PFF wonders if this is a reasonable place to put Experimental/Numerical Comparison Results?  For example, as here: (I think it is, yes -Dillon)}
...
\medskip
{\bf CHECK: Tzanio/Vladimir} MFEM data sources and models are needed here!! 
