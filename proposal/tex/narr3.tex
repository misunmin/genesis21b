...
\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{../figs/fig_fm.jpg}
    \caption{Schematic representation of the proposed method combining a foundation model with agentic AI. A wide range of high-quality fluid-flow datasets are used to train a foundation model, which encodes all the cases into a common latent space. Conditional latent diffusion is used to learn the distribution across cases in the latent space. Spatio-temporal data for a new geometry can be quickly produced in the latent space, and decoded back to the physical space. The agentic AI framework has access to this foundation model and the various optimizers, orchestrating the optimization/design process, which is significantly accelerated.}
    \label{fig:fm}
\end{figure}
...
{\bf Elia/Dillon/Vlad/Tzanio for preceding par.}
...
addressed in Phase II.) As illustrated in Fig. \ref{fig:fm}, test cases for
Phase I include optimization of pressure-drop/heat-transfer in wire-wrapped
fuel-rod bundles; a miniapp for ICF \cite[vlad4]; and optimization of
wire-coil inserts for heat transfer enhancement in beam-line devices.   These
test cases were selected because of the availability of reliable simulation and
experimental data and the fact that they can be scaled over a range of sizes,
with the number of simulation degrees-of-freedom (DOFs) ranging from $\sim
10^7$ to $10^{12}$ \cite{gb23arxiv,gb25mfem}.
...
\tiny \small \begin{verbatim}
  Provide a clear research plan for a 9-month project and describe the proposed
  ...
  the associated budget.  If the proposed application is building on an existing, 
  currently funded DOE project, describe how the submitted application leverages 
  that work and is distinct from the existing funding.

  Here building on Vlad and Ricardo's text, which will be integrated and
  supplemented with other material, including text from Sven,Paul,Misun.
\end{verbatim} 
\normalsize

The first component of the research plan focuses on the development of a
geometry-aware foundation model for thermal-fluid prediction, led by Argonne
National Laboratory. Building on prior work on foundation
models~\cite{Vishwasrao2026AgenticPDE}, as well as simulation and experimental
data from wire-wrapped bundles~\cite{Collins2022WireCoil} and related
configurations such as spacer grids, pebble beds, and twisted tubes, will be
mapped to a common latent space independent of mesh topology. This is
particularly critical for reactor applications, where geometry variations (wire
pitch, rod spacing, bundle configuration) strongly influence heat transfer and
pressure losses. The model will combine graph-based representations of
geometry, variational autoencoders for compressing high-dimensional flow
...
across operating regimes. Training will exploit available experimental data
sets (e.g., \cite{kaeri37pin,collins22}) and DOE exascale-ready workflows and
GPU-enabled datasets from Nek5000/RS and MFEM (Gordon Bell Prize winning codes
\cite{tufo99a,gb23,gb25mfem} developed under the Center for Efficient Exascale
Discretizations as part of DOE's Exascale Computing Project).  
The resulting surrogate will provide fast, accurate predictions of quantities
central to reactor design and licensing, including heat transfer coefficients,
pressure drop, and flow-induced mixing.
...
can be used to validate the ML surrogates.
...
experimental data show up to fourfold increases in heat transfer with
well-quantified pressure penalties . This problem is particularly valuable for
model development and validation due to its rich parameter space (wire
diameter, pitch, Reynolds number) and the strong agreement between simulations
and experiments. Together, these applications ensure both depth (reactor
...
through the Exascale Computing Project, particularly Nek5000/RS, which has been
extensively used for reactor thermal-hydraulics and is part of NRC-accepted
workflows. However, the proposed work is fundamentally distinct. Instead of
advancing simulation capabilities alone, we develop a generalizable foundation
model that unifies data across geometries and regimes, and we introduce a
closed-loop, AI-driven optimization framework that enables rapid design
exploration and mechanistic discovery. By the end of Phase I, we will
...
\tiny \small \begin{verbatim}
  ...
  Describe which existing or new data sources will be used. Address your team’s access 
  to the data.  If applicable, indicate which AI models or frameworks will be used in 
  your proposed project.  Do not include a computing resources--to be done in Appendix 2.

  Here, we discuss:
    o  Experimental data for fuel pins \cite{KAIST,Todreas}
    o  Nek5000/RS and MFEM as trusted high-accuracy and efficient sources
            - Top-ranked in numerous benchmark cases (T-junction \cite{tjunc},
              5-pin spacer grid \cite{5pin}, etc.)
            - 1/32 the amount of computational resources as FV for the same
              accuracy for Re=50M boundary layer calculation (1/8 the number
              of grid points, 1/2 the number of timesteps, and 1/2 the time
              per step, per gridpoint on the same platform, OLCF Summit)

    o  Collins data for the small wire-coil - both experimental and numerical
       results available \cite{collins22,kaneko23}
\end{verbatim} 
\normalsize
...
\cite{fischer08a,Dai2025WireWrapped61Pin,other} to full cores \cite{sc22,gb23,OTHER}.
